...
var actorChars = {
  ...
  function moveParticles() {
    ...
    context.fillText("START GAME BELOW!!", canvas.width/2, canvas.height/3);
...
function Enemy(pos) {
  this.flyPos = this.pos = pos.plus(new Vector(-1, -1));
  this.size = new Vector(1.8, 2);
  this.fly = Math.random() * Math.PI * 3;
}
Enemy.prototype.type = "enemy";

function Siron(pos) {
  this.flyPos = this.pos = pos.plus(new Vector(-.6, -.6));
  this.size = new Vector(1.7, 1.6);
  this.fly = Math.random() * Math.PI * 3;
}
Siron.prototype.type = "siron";
...
var wobbleSpeed = 3, wobbleDist = 0.07;
...
var flySpeed = 1, flyDist = 3;

Enemy.prototype.act = function(step) {
  this.fly += step * flySpeed;
  var flyPos = Math.sin(this.fly) * flyDist;
  this.pos = this.flyPos.plus(new Vector(flyPos, 0));
};

Siron.prototype.act = function(step) {
  this.fly += step * flySpeed;
  var flyPos = Math.sin(this.fly) * flyDist;
  this.pos = this.flyPos.plus(new Vector(flyPos, 0));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "siron" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin") {
